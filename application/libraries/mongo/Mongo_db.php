...
class Mongo_db {
	...
	*/
	...
	/**
	*	--------------------------------------------------------------------------------
	*	SAVE   by hfox 2014-08-20
	*	--------------------------------------------------------------------------------
	*
	*	save a single document
	*
	*	@usage: $this->mongo_db->save('foo', $data = array());
	*	如果是新增文档 将自动把新文档_id加入到$data中
	*/
	...
	public function save($collection = "", $data = array(), $options = array())
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected to save into", 500);
		}
